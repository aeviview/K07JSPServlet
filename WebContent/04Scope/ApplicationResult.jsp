...
<body>
	...
	<!-- 
		어플리케이션 영역에 저장된 속성은 페이지이동, 포워드 되더라도
		그 영역이 공유되고 웹브라우저를 종료했다가 다시 실행하더라도 그 영역이 공유된다.
		즉, 한 번 저장되면 웹서버가 셧다운(종료) 될 때까지는 속성값이 유지된다.
	 -->
	...
	<!-- 
		Main의 <h3>포워드로 페이지 전달하기</h3> 여기 부분을 먼저 주석처리하고 한다!
		
		 (1번실행: Result 실행 => 500에러남) (2번실행 : Main가서 실행함)
		 (3번실행 : 다시 Result와서 실행하면 에러가 사라짐)
		 (4번실행 : 모든 웹브라우저 종료하고 Result에서 실행하면 기억하고 창이 뜬다)
		 
		톰캣 웹서버를 완전히 종료(셧다운) 해버리고 이 페이지를 실행하면
		Nullpointer 예외가 발생하게 된다.
		아래 코드는 application 영역에 저장된 속성이 있다는 가정하에
		속성을 읽어오게 되는데 저장된 속성이 없기 때문에 오류가 발생하게 된다
	 -->
	...
	<h4>해당페이지에서만 사용가능하고, 페이지 이동 또는 forward되면 해당 영역은 사라진다</h4>
	<%
