...
<%-- 
	errorPage 지시어
		...
		설정한 페이지에서 하겠다는 선언.
		500에러와 같은 페이지를 사용자에게 보여주지 않게되므로
...
/*
해당 페이지로 폼값을 전송할 때 "나이" 부분을 공백상태 혹은 숫자가 아닌 문자를 입력 후 전송하면
해당 코드에서 에러가 발생한다.
*/
if(strAge != null)
{
	/*
	Integer.parseInt()숫자가 아닌 경우 예외가 발생된다.
	*/
...
<body>
	...
	<!-- 
		form태그의 속성
			...
			action : 폼 값이 전송될 웹서버의 경로. 혹은 URL.
				기술하지 않으면 현재 페이지로 폼 값이 전송된다.
			onsubmit : 전송 이벤트가 발생될 때를 감지하는 이벤트 리스너.
				주로 폼 값에 대한 검증을 위해 사용된다.
