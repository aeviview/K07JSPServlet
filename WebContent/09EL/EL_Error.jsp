...
<body>
	...
	<fieldset>
		...
		<!-- 
			...
			숫자로 변경할 수 없는 문자가 포함되었다면 예외가 발생한다.
		...
		<ul>
			...
			<li>${"HELLO" } ${"EL" } <!-- 문자열은 이렇게 표현하면 된다 --></li>
		</ul>
		
		<!-- 
			EL에서 속성에 접근시 -이 포함된 경우에는 .로 표현할 수 없고
			반드시 [를 통해서 표현해야 한다.
		 -->
		<h2>특수문자가 포함된 속성에 .으로 접근시</h2>
		<ul>
			<li>\${header["user-agent"] } : ${header["user-agent"] } [정상]</li>
			<li>\${header.user-agent } : ${header.user-agent } [에러NO, 출력 =0]</li>
		</ul>
		...
		<!-- 
			배열이나 컬렉션처럼 인덱스로 요소에 접근하는 경우에도 .은 사용할 수 없다
		 -->
		<ul>
		...
		<!-- 
			...
			해당 문장은 배열이나 컬렉션의 크기만큼 반복하는 확장 for문 형태로 기술되었다.
		 -->
		...
		<!-- 
			인덱스를 초과하면 예외가 발생되어야 하지만
			EL에서는 기본적인 예외를 잡아주므로 에러가 발생되지 않는다.
		 -->
		<h2>인덱스를 벗어난 경우</h2>
		...
		<%
		MemberDTO member = new MemberDTO("SONG", "1234", "송길동", null);
		%>
		<c:set var="eldto" value="<%=member %>" />
		
		<ul>
			<li>존재하지 않는 속성 : \${eldto.addr } : \${eldto.addr } [PropertyNotFoundException발생]</li>
			<!-- MemberDTO에 getId()를 주석처리한 후 테스트 해볼 것 -->
			<li>속성은 존재, getter가 없는 경우 : ${eldto.id } : \${eldto.id } [500에러발생]</li>
			<li>el변수명이 틀린 경우 : \${noeldto.pwd } : ${noeldto.pwd } [에러NO, 미출력]</li>
		</ul>
		
		<h2>EL내장객체의 없는 속성으로 접근시</h2>
		<ul>
			<li>\${requestScope.nop } : ${requestScope.nop }
				[Scope계열 내장객체, 에러NO, 미출력 => 속성명은 개발자 마음대로 정할 수 있다]
			</li>
			<!-- 
			...
			<li>\${pageContext.nop } : \${pageContext.nop }
				[컴파일 오류발생 => 내장객체명은 정해져 있기 때문이다]
			</li>
		</ul>
		
		<h2>EL의 내장객체 중 사용자가 정의한 속성에 접근하는 내장객체 및 맵 컬렉션인 경우</h2>
		<%
		...
		<h2>EL 변수에 값 할당시 [톰켓8.0부터는 정상. 이전버전은 에러난다]</h2>
