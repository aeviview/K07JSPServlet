...
<body>
	...
	<ul>
		...
		<li>오라클 접속 URL : <%=application.getInitParameter("ConnectionURL") %></li>
	</ul>
	...
	<ul>
		<li>application 내장객체 : <%=application.getRealPath("/images") %></li>
		<li>request 내장객체 : <%=request.getServletContext().getRealPath("/images") %></li>
		<li>request 내장객체(주로 서블릿에서 사용) : <%=request.getRealPath("/images") %></li>
		<li>session 내장객체 : <%=session.getServletContext().getRealPath("/images") %></li>
		<li>config 내장객체 : <%=config.getServletContext().getRealPath("/images") %></li>
	...
	반드시 매개변수 형태로 전달한 후 내장객체의 클래스형으로 받은 다음 사용해야 한다.
	*/
