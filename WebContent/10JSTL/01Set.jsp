...
<body>
	...
	<!-- 
		[set태그 속성]
			var : 변수명으로 문자열만 가능하다
			value : 값을 설정함. 표현식, EL식 모두 가능하다
			scope : 영역을 지정할 수 있고, 기본값은 가장 좁은 page영역이다
	...
	<h3>EL로 출력</h3>
	<!-- 
		EL로 영역의 속성을 읽어올 때는 xxScope를 생략하면 가장 좁은 영역인
		page영역을 읽어오게 되므로, 속성명이 동일하지 않다면 내장객체는 생략 가능하다.
	 -->
	<ul>
	...
	<h3>JSP코드로 출력</h3>
	<!-- 
		영역에 저장될 때 Object타입으로 자동형변환 되므로
		사용시 해당 자료형으로 캐스팅(형변환) 해야한다.
	 -->
	<ul>
	...
	<!-- 
		...
		컬렉션의 값을 설정할 때 사용하는 속성이다.
			target : 변수값을 지정하는 속성. 반드시 EL, 표현식만 가능하다
			property : 값을 지정하는 속성. EL, 표현식, 특정한 값 모두 가능하다
				- 자바빈인 경우 : 속성명(멤버변수)
				- Map컬렉션인 경우 : key값
	...
	<!-- 
		Map계열의 컬렉션은 index값이 아니라 key값을 통해 value에 접근한다.
	...
	<c:set target="${map.args }" property="id" value="twice" />
	<c:set target="${map.args }" property="pass" value="1111" />
	<c:set target="${map.args }" property="name" value="트와이스" />
