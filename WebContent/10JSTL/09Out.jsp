...
<body>
	...
	<!-- 
		- out태그에 value속성의 값이 null일 때는 default속성에 지정된 값이 출력.
	...
	<!-- 
		...
		- 예를 들어 주소창의 nowPage=3으로 하면 3페이지로 가듯이
			default값을 3으로 하면 첫 실행창에 3페이지가 나온다!
	 -->
	<h3>페이지 링크에 응용하기</h3>
	<c:url value="/08Board1/BoardList.jsp?nowPage=" /><c:out value="${param.nowPage }" default="3" />
	<hr />
	<!-- 주의 : 한줄로 붙여서 작성하시오. 줄바꿈하면 공백때문에 err남! -->
	<a href="<c:url value="/08Board1/BoardList.jsp?nowPage=" /><c:out value="${param.nowPage }" default="3" />">
	회원제게시판 리스트 바로가기</a>
